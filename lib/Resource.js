...
Resource.prototype._checkExists  = function(resource) {
  if (resource instanceof Resource) {
    if (!resource._base.id){
      return false
    }
  }else{
...
Resource.prototype.relationships = function(relationName) {
  ...
  return {
    match: function(resources){
      var relation = self[relationName] || [];
      const toRemove = relation.filter(resource  => { return !resources.find( el=> {return el._raw.id === resource._raw.id}) })
      const toAdd    = resources.filter(resource => { return !relation.find( el=> {return el._raw.id === resource._raw.id}) })
...
Resource.prototype._removeRelation = function(relationName, rawRelation, resource) {
  ...
  if (Resource._rawRelationHasMany(rawRelation)) {
    ...
    rawRelation.data = null
    delete rawRelation.links
    delete rawRelation.meta
