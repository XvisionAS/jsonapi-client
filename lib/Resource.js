...
Resource.prototype.relationships = function(relationName) {
  ...
  return {
    match: function(resources){
      ...
      const toRemove = relation.filter(resource  => { return !resources.find( el=> {return el.id === resource.id}) })
      const toAdd    = resources.filter(resource => { return !relation.find( el=> {return el.id === resource.id}) })
